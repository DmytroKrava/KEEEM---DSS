...
import { Map as LeafletMap, TileLayer, Pane } from 'react-leaflet';
...
import { tooltip, modal_window, changeRegime } from "../interactive-hints/interactive-hints.js";
    ...
    <>
      ...
      {user && (
        <Button
          ...
          data-content={isAddPointModeEnabled ? 'Припинити додавання маркерів на карту' : 'Включити режим додавання маркера'}
          onMouseMove={tooltip}
      ...
      {user && (
        <Button
          ...
          data-content={isAddPolygonModeEnabled ? 'Після виключення полігон не буде збережено' : 'Включити режим додавання полігону'}
          onMouseMove={tooltip}
      ...
      {isAddPolygonModeEnabled && (
        <Button
          ...
          data-content="Зупинити прокладення полігона й зберегти його на карті" 
          onMouseMove={tooltip}
        >
          Закінчити полігон
        </Button>
      ...
      {user && (
        <Button
          ...
          data-content={isAddTubeMode ? 'Після виключення трубу не буде збережено' : 'Включити режим додавання труби'}
          onMouseMove={tooltip}
          disabled={
      ...
      {isAddTubeMode && (
        <Button
          ...
          variant='outline-success'
          onClick={finishTube}
          data-content="Зупинити прокладення труби й зберегти її на карті" 
          onMouseMove={tooltip}
        >
          Закінчити трубу
        </Button>
     ...
     {isCompareMode && (
      ...
      <Button
        ...
        data-content="Натисніть, щоб переглянути результати порівняння"
        onMouseMove={tooltip}
      ...
      <button 
        className="btn btn-info sticky-button-wrapper modal-window-button" 
        data-content="<p>Контентна частина сторінки з картою викидів складається з карти та 
        нижньої навігаційної панелі, також є додаткові бокові панелі як відкриваються лише при 
        натисканні та містять окремий функціонал.</p>
        <p>На сторінці карти розміщені два бокові вікна, які відкриваються коли 
        натиснути на прапорці що знаходяться по бокам екрану, зліва та справа, натиснувши на 
        них, відкриється одне з вікон, вікно фільтрації яке розміщене з лівої сторони екрану 
        або пошуку яке розміщено з правої сторони екрану, одночасно може бути відкрито лише 
        одне бокове вікно.</p>
        <p>На цій карті ви маєте можливість ставити мітки, поілгони тощо.</p>
        <p>Детальну інформацію про меню 'Карта викидів' ви можете знайти в інструкції.</p>" onClick={modal_window}>
...
const UpButton = ({show})=>{
  return(
      <div 
          ...
          data-content="Обрати проміжки актуальності даних. На карті будуть виведені елементи, які актуальні для обраних часових проміжків."
          onMouseMove={tooltip}
      />
  )
}
